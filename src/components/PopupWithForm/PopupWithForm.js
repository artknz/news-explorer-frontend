...
export default function PopupWithForm({name, title, isOpen, onClose, onRegisterClick, onAuthClick, onSubmit, buttonText, linkText, children}) {
  return(
    <section className={`popup popup_${name} ${isOpen && 'popup_opened'}`}>
      <form name={name} onSubmit={onSubmit} className={`popup__container popup__container_form popup_container_type_${name}`}>
        ...
        <p className="popup__alt">или<button type="button" onClick={onRegisterClick || onAuthClick} className="popup__route">{linkText}</button></p>
      </form>
    </section>
  );
};
