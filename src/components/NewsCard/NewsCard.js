...
export default function NewsCard({ urlToImage, channelName, title, text, date, onSave, id, onDelete, keyword }) {
  const location = useLocation();

  function handleButton() {
    if (location.pathname === '/saved-news') {
      return <div>
        <button onClick={onDelete} type="button" className={'news-card__delete'} />
